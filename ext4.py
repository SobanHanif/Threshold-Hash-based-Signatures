...
prevents excessive precomputation by hiding/waiting for a call on a certain message before generating the tree holding it
"""
import hashlib
from typing import Callable

from merkle import build_merkle, merkle_auth_path
...
class SubTree:
  def __init__(self, keygen_fn: Callable, sign_fn: Callable, hash_fn: Callable, size):
    ...
    self._levels = build_merkle(leaves)
    self.root: bytes = self._levels[-1][0]

  def exhausted(self):
    return self._used >= self._size

  # signature consists of the index, signature, specific public key and proof
  def sign(self, message):
    ...
    path = merkle_auth_path(self._levels, idx)
    return idx, sig, self._pks[idx], path

class HyperTree:
  def __init__(self, keygen_fn: Callable, sign_fn: Callable, verify_fn: Callable, hash_fn: Callable, subtree_size, num_layers,
  ):
    if num_layers < 2:
    ...
    self._layers[top_layer_idx] = SubTree(keygen_fn, sign_fn, hash_fn, subtree_size)
  ...
  def sign(self, message: bytes) -> dict:
    # If a lower layer is exhausted, generate new layer & sign its new root using the above layer
    for d in range(self._num_layers - 2, -1, -1):
      if self._layers[d] is None or self._layers[d].exhausted():
        self._layers[d] = SubTree(
          self._keygen_fn, self._sign_fn, self._hash_fn, self._subtree_size
        )
...
def verify_hyper(signature: dict, cpk: bytes, verify_fn: Callable, hash_fn: Callable):
  ...
  for depth in range(len(signature["sigs"])):
    if not verify_fn(signature["sigs"][depth], signature["pks"][depth], current_msg):
      return False
    
    leaf = hash_fn(signature["pks"][depth])
    root = _walk(leaf, signature["key_indices"][depth], signature["auth_paths"][depth])

    current_msg = root

  return current_msg == cpk

# this is just verify root from merkle, too lazy to move into there
def _walk(leaf, index, path):
  cur = leaf
  idx = index
  for sibling in path:
    if idx % 2 == 0:
